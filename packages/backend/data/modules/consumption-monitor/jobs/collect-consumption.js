...
import { scrapingService } from '../../../../src/services/scraping.service.js';

export default async function collectConsumption(context) {
  const { logger, prisma } = context.services;
  const { batchSize = 5, screenshotOnError = true } = context.config || {};

  logger.info('Starting consumption collection job');
  ...
  try {
    ...
    if (endpoints.length === 0) {
      logger.info('No enabled endpoints found');
      return {
        success: true,
    ...
    logger.info(`Found ${endpoints.length} enabled endpoints to process`);
    ...
    for (let i = 0; i < endpoints.length; i += batchSize) {
      ...
      logger.info(`Processing batch ${Math.floor(i / batchSize) + 1} (${batch.length} endpoints)`);

      // Process batch concurrently
      await Promise.all(
        batch.map(endpoint => processEndpoint(endpoint, logger, prisma, screenshotOnError, results))
      );
    }
    ...
    logger.info(`Consumption collection completed. Success: ${results.successful}, Failed: ${results.failed}`);
    ...
    logger.error(`Consumption collection job failed: ${error.message}`, { error });
 ...
 */
async function processEndpoint(endpoint, logger, prisma, screenshotOnError, results) {
  const startTime = Date.now();

  try {
    logger.info(`Scraping endpoint: ${endpoint.name} (${endpoint.ipAddress})`);
    ...
    logger.info(
      `Successfully scraped ${endpoint.name}: ${result.value} kWh (current month: ${currentKwh.toFixed(2)} kWh)`
    );

    results.successful++;
  } catch (error) {
    logger.error(`Failed to scrape ${endpoint.name}: ${error.message}`, {
      endpointId: endpoint.id,
      error,
    });
    ...
    try {
      ...
      logger.error(`Failed to store error reading: ${dbError.message}`);
    }
